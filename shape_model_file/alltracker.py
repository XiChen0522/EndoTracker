...
if not _shape_logger.handlers:
    _fh = _logging.FileHandler("shape_trace.log", mode="w", encoding="utf-8")
    _fh.setFormatter(_logging.Formatter("%(message)s"))
    _shape_logger.addHandler(_fh)

def _slog(msg):
    """写入 shape_trace.log，同时打印到终端的精简版"""
    _shape_logger.info(msg)

# 控制: 第几个window打印完整trace，其余只打印摘要
_first_window_done = False
_first_iter_done = False  # 在每个window内，第一次iter打印完整，后续只打印delta
# ================================
...
class Net(nn.Module):
    ...
    def forward_sliding(self, images, iters=4, sw=None, is_training=False, window_len=None, stride=None):
        ...
        _slog(f"\n{'='*70}")
        _slog(f"[SHAPE] forward_sliding")
        _slog(f"  images: ({B}, {T}, {C}, {H}, {W})")
        _slog(f"  S={S}, stride={stride}, iters={iters}")
        print(f"[SHAPE TRACE] Output写入 shape_trace.log (images={B}x{T}x{C}x{H}x{W}, S={S})")
        ...
        if self.no_split:
            C = self.dim
            C2 = C
        _slog(f"  after pad: H_pad={H_pad}, W_pad={W_pad}, H8={H8}, W8={W8}")
        _slog(f"  C={C}, C2={C2}, no_split={self.no_split}, T(padded)={T}")
    ...
    def forward_window(self, fmap1_single, fmaps2, visconfs8, iters=None, flowfeat=None, flows8=None, sw=None, is_training=False):
        ...
        _full = not _first_window_done  # 第一个window打印完整信息
        ...
        if _full:
            _slog(f"\n{'='*70}")
            _slog(f"[forward_window] B={B}, S={S}, C={C}, H8={H8}, W8={W8}, iters={iters}")
        ...
        if _full:
            _slog(f"  fmap1 (anchor*S): {tuple(fmap1.shape)}  = (B*S={B*S}, C={C}, H8={H8}, W8={W8})")
            _slog(f"  fmap2 (targets):  {tuple(fmap2.shape)}")
        ...
        if _full:
            _slog(f"  CorrBlock: levels={self.corr_levels}, radius={self.corr_radius}, corr_ch={self.corr_channel}")
            _slog(f"  coords1 (grid): {tuple(coords1.shape)}")
        ...
        if _full:
            _slog(f"  flowfeat: {tuple(flowfeat.shape)}, ctxfeat: {tuple(ctxfeat.shape)}")
        ...
        if _full:
            _slog(f"  time_emb: {tuple(time_emb.shape)}")
            _slog(f"  ctxfeat (+time_emb): {tuple(ctxfeat.shape)}")
        ...
        for itr in range(iters):
            _detail = _full and not _first_iter_done  # 只在第一个window的第一次iter打印子模块细节
            ...
            if _full:
                _slog(f"  motion: {tuple(motion.shape)}  (pdim={self.pdim})")
                if _detail:
                    _slog(f"\n  >>> update_block Input:")
                    _slog(f"      flowfeat:  {tuple(flowfeat.shape)}")
                    _slog(f"      ctxfeat:   {tuple(ctxfeat.shape)}")
                    _slog(f"      visconfs8: {tuple(visconfs8.shape)}")
                    _slog(f"      corr:      {tuple(corr.shape)}")
                    _slog(f"      motion:    {tuple(motion.shape)}")
                    _slog(f"      S={S}")

            # 传递_detail标志给blocks (通过全局变量)
            import nets.blocks as _blk
            _blk._trace_detail = _detail
            ...
            if _full:
                if _detail:
                    _slog(f"  <<< update_block Output: flowfeat={tuple(flowfeat.shape)}")
                    _slog(f"  flow_update:    {tuple(flow_update.shape)}")
                    _slog(f"  visconf_update: {tuple(visconf_update.shape)}")
                    _slog(f"  weight_update:  {tuple(weight_update.shape)}")
                    _slog(f"  flow_up (8x):   {tuple(flow_up.shape)}")
                else:
                    _slog(f"  (iter {itr} same shapes, skipped)")
