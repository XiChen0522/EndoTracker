...
# ====== Shape Trace (shared with alltracker) ======
import logging as _logging
_shape_logger = _logging.getLogger("shape_trace")  # same logger name -> same file

def _slog(msg):
    _shape_logger.info(msg)

# 由alltracker.forward_window设置: True=打印子模块细节, False=静默
_trace_detail = False
# ==================================================
...
class CNBlock1d(nn.Module):
    ...
    def forward(self, input, S=None):
        if self.dense:
            ...
            if _trace_detail:
                _slog(f"    [CNBlock1d Temporal attn]")
                _slog(f"      Input: ({BS},{C},{H},{W}) = (B*S, C, H, W)")

            input = einops.rearrange(input, '(b s) c h w -> (b h w) c s', b=B, s=S, c=C, h=H, w=W)

            if _trace_detail:
                _slog(f"      rearrange -> ({B*H*W}, {C}, {S}) = (B*H*W, C, S)")

            if self.use_mixer or self.use_attn:
                # mixer/transformer blocks want B,S,C
                if _trace_detail:
                    _slog(f"      permute -> ({B*H*W}, {S}, {C}) for {'attn' if self.use_attn else 'mixer'}")
            ...
            if _trace_detail:
                _slog(f"      Output: {tuple(result.shape)} = (B*S, C, H, W)")
        else:
            B,S,C = input.shape
...
class CNBlock2d(nn.Module):
    ...
    def forward(self, input, S=None):
        if _trace_detail:
            _slog(f"    [CNBlock2d Spatialconv] Input: {tuple(input.shape)} (B*S,C,H,W) 7x7 depthwise")
        result = self.layer_scale * self.block(input)
        result = self.stochastic_depth(result)
        result += input
        result = self.final(result)
...
class BasicMotionEncoder(nn.Module):
    ...
    def forward(self, flow, corr):
        if _trace_detail:
            _slog(f"    [MotionEncoder] flow: {tuple(flow.shape)}, corr: {tuple(corr.shape)}")
        ...
        if self.pdim==2 or self.pdim==4:
            ...
            result = torch.cat([out, flow], dim=1)
            if _trace_detail:
                _slog(f"      cor: {tuple(cor.shape)}, flo: {tuple(flo.shape)}")
                _slog(f"      output: {tuple(result.shape)} = conv_out({out.shape[1]}) + flow({flow.shape[1]})")
            return result
        else:
            # the flow is already encoded to something nice
            cor_flo = torch.cat([cor, flow], dim=1)
            result = F.relu(self.conv(cor_flo))
            if _trace_detail:
                _slog(f"      cor: {tuple(cor.shape)}, cor_flo: {tuple(cor_flo.shape)}")
                _slog(f"      output: {tuple(result.shape)} (sinenc path)")
            return result
...
class RelUpdateBlock(nn.Module):
    ...
    def forward(self, flowfeat, ctxfeat, visconf, corr, flow, S, upsample=True):
        ...
        B = BS//S
        if _trace_detail:
            _slog(f"\n  [RelUpdateBlock] BS={BS}, C={C}, H={H}, W={W}, B={B}, S={S}")
        motion_features = self.motion_encoder(flow, corr)
        if _trace_detail:
            _slog(f"    motion_features: {tuple(motion_features.shape)}")
        if self.no_ctx:
            cat_tensor = torch.cat([flowfeat, motion_features, visconf], dim=1)
            if _trace_detail:
                _slog(f"    compressor input(no_ctx): {tuple(cat_tensor.shape)} = feat({C})+mot({motion_features.shape[1]})+vis({visconf.shape[1]})")
            flowfeat = self.compressor(cat_tensor)
        else:
            cat_tensor = torch.cat([flowfeat, ctxfeat, motion_features, visconf], dim=1)
            if _trace_detail:
                _slog(f"    compressor input: {tuple(cat_tensor.shape)} = feat({C})+ctx({ctxfeat.shape[1]})+mot({motion_features.shape[1]})+vis({visconf.shape[1]})")
            flowfeat = self.compressor(cat_tensor)
        if _trace_detail:
            _slog(f"    after compressor: {tuple(flowfeat.shape)}")
        for blk_idx, blk in enumerate(self.refine):
            flowfeat = blk(flowfeat, S)
            if _trace_detail:
                _slog(f"    after refine[{blk_idx}]({blk.__class__.__name__}): {tuple(flowfeat.shape)}")
        flowfeat = self.final_conv(flowfeat)
        if _trace_detail:
            _slog(f"    after final_conv: {tuple(flowfeat.shape)}")
        return flowfeat