...
_ct3on_logger = _logging.getLogger("shape_trace_ct3_online")
_ct3on_logger.setLevel(_logging.DEBUG)
_ct3on_logger.propagate = False
if not _ct3on_logger.handlers:
    _fh = _logging.FileHandler("cotracker3_online_shape_trace.log", mode="w", encoding="utf-8")
    _fh.setFormatter(_logging.Formatter("%(message)s"))
    _ct3on_logger.addHandler(_fh)
def _slog(msg):
    _ct3on_logger.info(msg)
...
class CoTrackerThreeOnline(CoTrackerThreeBase):
    ...
    def forward_window(
        ...
        if _full:
            _slog(f"\n{'='*70}")
            _slog(f"[forward_window] B={B}, S={S}, N={N}, iters={iters}")
            _slog(f"  fmaps_pyramid levels: {[tuple(f.shape) for f in fmaps_pyramid]}")
            _slog(f"  coords: {tuple(coords.shape)}  = (B, S, N, 2)")
            _slog(f"  track_feat_support_pyramid: {[tuple(t.shape) for t in track_feat_support_pyramid]}")
            _slog(f"  vis: {tuple(vis.shape)}, conf: {tuple(conf.shape)}")
            _slog(f"  attention_mask: {tuple(attention_mask.shape)}")
            _slog(f"  r (support size): {r}, corr_radius={self.corr_radius}")
        ...
        for it in range(iters):
            ...
            for i in range(self.corr_levels):
                ...
                if _detail and i == 0:
                    ...
                    _slog(f"  Correlation (level 0):")
                    _slog(f"    corr_feat: {tuple(corr_feat.shape)}  = (B, S, N, {r}, {r}, latent_dim)")
                    _slog(f"    track_feat_support: {tuple(track_feat_support.shape)}  = (B, N, {r}, {r}, latent_dim)")
                    _slog(f"    corr_volume: {tuple(corr_volume.shape)}  = (B, S, N, {r}, {r}, {r}, {r})")
                    _slog(f"    reshape: ({B*S*N}, {r**4}) -> corr_mlp -> corr_emb: {tuple(corr_emb.shape)}")

            corr_embs = torch.cat(corr_embs, dim=-1)
            corr_embs = corr_embs.view(B, S, N, corr_embs.shape[-1])

            if _detail:
                _slog(f"    corr_embs (all levels): {tuple(corr_embs.shape)}  = (B, S, N, {corr_embs.shape[3]})")
            ...
            if _detail:
                _slog(f"    rel_pos_emb: {tuple(rel_pos_emb_input.shape)}")
            ...
            if _detail:
                _slog(f"\n  Token construction ({self.input_dim}-dim):")
                _slog(f"    vis:         {vis.shape[-1]}")
                _slog(f"    conf:        {conf.shape[-1]}")
                _slog(f"    corr_embs:   {corr_embs.shape[3]}  ({self.corr_levels} levels x {corr_emb.shape[-1]})")
                _slog(f"    rel_pos_emb: {rel_pos_emb_input.shape[3]}")
                _slog(f"    total:       {x.shape[2]}")
                _slog(f"  x (before time_emb): {tuple(x.shape)}  = (B*N={B*N}, S={S}, input_dim)")

            x = x + self.interpolate_time_embed(x, S)
            x = x.view(B, N, S, -1)  # (B N) T D -> B N T D

            if _detail:
                _slog(f"  x (after +time_emb): {tuple(x.shape)}  = (B, N, S, input_dim)")
                _slog(f"\n  >>> updateformer input: {tuple(x.shape)}")

            delta = self.updateformer(x, add_space_attn=add_space_attn)

            if _detail:
                _slog(f"  <<< updateformer output: {tuple(delta.shape)}  = (B, N, S, {delta.shape[3]})")
            ...
            if _detail:
                ...
                _slog(f"  delta_vis: {tuple(delta_vis.shape)}  = (B, S, N, 1)")
                _slog(f"  delta_conf: {tuple(delta_conf.shape)}  = (B, S, N, 1)")
                _slog(f"  updated coords (pixels): {tuple(coord_preds[-1].shape)}")
            elif _full:
    ...
    def forward(
        ...
        _slog(f"\n{'='*70}")
        _slog(f"[SHAPE] CoTrackerThreeOnline.forward()")
        _slog(f"  video: ({B}, {T}, {C}, {H}, {W})")
        _slog(f"  queries: ({B}, {N}, {queries.shape[2]})")
        _slog(f"  stride={self.stride}, corr_levels={self.corr_levels}, corr_radius={self.corr_radius}")
        _slog(f"  latent_dim={self.latent_dim}, input_dim={self.input_dim}")
        _slog(f"  num_virtual_tracks={self.num_virtual_tracks}")
        _slog(f"  is_online={is_online}")
        print(f"[SHAPE TRACE] Output -> cotracker3_online_shape_trace.log (video={B}x{T}x{C}x{H}x{W}, N={N})")
        ...
        _slog(f"  S={S}, step={S//2}, T_pad={T_pad}")
        _slog(f"  H4={H//self.stride}, W4={W//self.stride}")
        ...
        _slog(f"  fmaps (after L2 norm): {tuple(fmaps.shape)}  = (B, T_pad, latent_dim, H4, W4)")
        ...
        _slog(f"\n  Track features (per level):")
        for i in range(self.corr_levels):
            _slog(f"    level {i}: feat={tuple(track_feat_pyramid[i].shape)}, support={tuple(track_feat_support_pyramid[i].shape)}")
        ...
        _slog(f"\n  num_windows={num_windows}, step={step}")
        _slog(f"  coords_init: {tuple(coords_init.shape)}")
        ...
        for ind in indices:
            _slog(f"\n  --- window ind={ind}, frames={ind}~{ind+S-1} ---")
            if ind > 0:
                overlap = S - step
        ...
        _slog(f"\n  Final output:")
        _slog(f"    coords_predicted: {tuple(coords_predicted.shape)}")
        _slog(f"    vis_predicted: {tuple(vis_predicted.shape)}")
        _slog(f"    conf_predicted: {tuple(conf_predicted.shape)}")
